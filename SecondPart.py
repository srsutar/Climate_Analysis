...
@cli_app.route("/")
def Welcome_page():
    return (f"Welcome to Tropical Hawaiian Climate Analysis!!!<br/>" 
        f"Pathways to Discover<br/>"
        f"/precipitation_data<br/>"
        f"/station_data<br/>"
        f"/temperature_observed<br/>"
    )
...
def precipitation_data():
 #date for previous year
    ...
    preci_scores = session.query(measurement.date,measurement.prcp).filter(measurement.date>=prev_year).all()
    #create a dict. with date as key and pre as value
    prec = {date: prcp for date, prcp in preci_scores}
    return jsonify(prec)
